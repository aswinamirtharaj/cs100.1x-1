...
groupedByDay = dayAndHostTuple.groupByKey().map(lambda (x, y): (x, len(y)))

sortedByDay = groupedByDay.sortByKey()

avgDailyReqPerHost = (sortedByDay
                      .join(dailyHosts).map(lambda (k,(v1,v2)): (k, v1/v2))
                      .sortByKey()
                      .cache())
...
badEndpoints = badRecords.map(lambda rec: rec.endpoint)
...
badEndpointsCountPairTuple = badRecords.map(lambda rec: (rec.endpoint, 1))
...
errHostsCountPairTuple = badRecords.map(lambda rec: (rec.host, 1))
...
errHourList = hourRecordsSorted.take(30)
