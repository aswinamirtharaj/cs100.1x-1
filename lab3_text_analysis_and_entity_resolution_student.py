...
def simpleTokenize(string):
    ...
    if not string.strip():
        ...
        return re.split(split_regex, string.strip(' !"#$%&\'()*+,-./:;<=>?@[\]^`{|}~').lower())
...
def tokenize(string):
    ...
    """
    simpleTokens = simpleTokenize(string)
    return [token for token in simpleTokens if token not in stopwords]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda (id, x): (id, tokenize(x)))
googleRecToToken = googleSmall.map(lambda (id, x): (id, tokenize(x)))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (id, x): len(x)).reduce(lambda a, b: a + b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, lambda (id, ntokens): -1 * len(ntokens))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (id, tokens): list(set(tokens))).filter(lambda x: x != '')
    tokenCountPairTuple = uniqueTokens.map(lambda token: (token, 1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b)

    return (tokenSumPairTuple.map(lambda (token, idf): (token, 1. * N / idf)))
...
avgSimDups = trueDupsRDD.map(lambda (x, (y, z)): y).reduce(lambda a, b: a+b)/trueDupsCount

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda (x, (y, z)): z == None))

avgSimNon = nonDupsRDD.map(lambda (x, (y, z)): y).reduce(lambda a, b: a+b)/nonDupsRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (id, x): (id, tokenize(x)))
googleFullRecToToken = google.map(lambda (id, x): (id, tokenize(x)))
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast.value.get(amazonRec)[t] * googleWeightsBroadcast.value.get(googleRec)[t] for t in tokens])
    value = s/(amazonNormsBroadcast.value[amazonRec])/(googleNormsBroadcast.value[googleRec])
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(lambda amIDGoogUrlToken: fastCosineSimilarity(amIDGoogUrlToken))
                       .cache())
