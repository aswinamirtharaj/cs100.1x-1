...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    nRatings = 1. * len(ratings)
    return (movieID, (nRatings, sum(ratings)/nRatings))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (id, (name, (num, rating))): (rating, name, num)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (rating, name, num): num > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda ((a, b), (x, y)): (x - y)**2))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.reduce(lambda a, b: a + b)

    # Count the number of entries for which you computed the total squared error
    numRatings = 1.*squaredErrorsRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
trainingAvgRating = trainingRDD.map(lambda (x, y, z): z).reduce(lambda a, b: a + b)/trainingRDD.count()
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda (x, y): (0, x))
                      .filter(lambda (a, x): x not in [a[1] for a in myRatedMovies]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (id, ((pred, num), name)): (pred, name, num))
                       .filter(lambda (pred, name, num): num > 75))
